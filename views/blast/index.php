            ...
            <form method="POST" action="<?= BASE_URI ?>/blast/send" id="blast-form" enctype="multipart/form-data" class="px-6 py-5 space-y-5">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        Provider Penghantaran
                    </label>
                    <div class="grid grid-cols-2 gap-3">

                        <!-- Fonnte -->
                        <label class="provider-card relative flex flex-col gap-1.5 border-2 rounded-xl p-3.5 cursor-pointer transition-all
                                      <?= $configured ? 'hover:border-green-400' : 'opacity-50 cursor-not-allowed' ?>
                                      border-green-500 bg-green-50 dark:bg-green-900/20"
                               id="card-fonnte">
                            <input type="radio" name="provider" value="fonnte"
                                   <?= !$configured ? 'disabled' : 'checked' ?>
                                   class="sr-only" onchange="selectProvider('fonnte')">
                            <div class="flex items-center gap-2">
                                <svg class="w-4 h-4 text-green-600 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                                </svg>
                                <span class="text-sm font-semibold text-gray-800 dark:text-white">Fonnte</span>
                                <?php if ($configured): ?>
                                <span class="ml-auto text-xs bg-green-500 text-white px-1.5 py-0.5 rounded-md font-medium">Aktif</span>
                                <?php endif; ?>
                            </div>
                            <p class="text-xs text-gray-500 dark:text-gray-400">Hantar via nombor WA anda sendiri. Boleh kena restrict.</p>
                        </label>

                        <!-- WhatsApp Cloud API -->
                        <label class="provider-card relative flex flex-col gap-1.5 border-2 rounded-xl p-3.5 cursor-pointer transition-all
                                      <?= $waApiReady ? 'hover:border-blue-400' : 'opacity-60' ?>
                                      <?= $waApiReady ? 'border-gray-200 dark:border-gray-600' : 'border-gray-200 dark:border-gray-700' ?>"
                               id="card-whatsapp_api">
                            <input type="radio" name="provider" value="whatsapp_api"
                                   <?= !$waApiReady ? 'disabled' : '' ?>
                                   class="sr-only" onchange="selectProvider('whatsapp_api')">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-semibold text-gray-800 dark:text-white">WA Business API</span>
                                <?php if ($waApiReady): ?>
                                <span class="ml-auto text-xs bg-blue-500 text-white px-1.5 py-0.5 rounded-md font-medium">Aktif</span>
                                <?php else: ?>
                                <span class="ml-auto text-xs bg-gray-200 dark:bg-gray-700 text-gray-500 px-1.5 py-0.5 rounded-md font-medium">Belum setup</span>
                                <?php endif; ?>
                            </div>
                            <p class="text-xs text-gray-500 dark:text-gray-400">Meta official API. Selamat, tak kena ban. Perlukan Meta Business account.</p>
                            <?php if (!$waApiReady): ?>
                            <p class="text-xs text-blue-600 dark:text-blue-400 font-medium mt-0.5">→ Tambah WA_PHONE_NUMBER_ID + WA_ACCESS_TOKEN dalam config.php</p>
                            <?php endif; ?>
                        </label>
                ...
                <div class="px-5 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors <?= $clickable ? 'cursor-pointer' : '' ?>"
                    ...
                    <div class="flex items-center justify-between mb-1">
                        <div class="flex items-center gap-2">
                            ...
                            <?php if (($log['provider'] ?? 'fonnte') === 'whatsapp_api'): ?>
                            <span class="text-xs px-1.5 py-0.5 rounded bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300">WA API</span>
                            <?php endif; ?>
                        </div>
...
function selectProvider(p) {
    ['fonnte','whatsapp_api'].forEach(function(name) {
        var card = document.getElementById('card-' + name);
        if (!card) return;
        if (name === p) {
            card.classList.remove('border-gray-200','dark:border-gray-600','dark:border-gray-700');
        ...
        if (radio && !radio.disabled) radio.checked = (name === p);
    });
}
